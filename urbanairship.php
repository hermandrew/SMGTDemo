<link rel="stylesheet" href="reset-min.css"><?php
...
define('RICH_BROADCAST_URL', BASE_URL . '/airmail/send/broadcast/');
...
class Airship {
    ...
    public function push($payload, $device_tokens=null, $aliases=null, $tags=null) {
        ...
        $response_code = $response[0];
		echo "BlackBerry Status Code: " . $response_code . "<br><br>";
        if ($response_code != 200) {
            throw new AirshipFailure($response[1], $response_code);
        }
    }
    ...
    public function broadcast($payload, $exclude_tokens=null) {
        ...
        $response_code = $response[0];
        echo "Blackberry Broadcast Status Code: " . $response_code . "<br><br>";
        if ($response_code != 200) {
            throw new AirshipFailure($response[1], $response_code);
        }
    }
    ...
    // Send Rich Push
    public function richPush($payload, $users=null, $aliases=null, $tags=null) {
    	if ($users != null) {
            $payload['users'] = $users;
        }
        if ($aliases != null) {
        ...
        $body = json_encode($payload);
        $response = $this->_request(RICH_PUSH_URL, 'POST', $body, 'application/json');
        $response_code = $response[0];
        echo "Rich Push Status Code: " . $response_code . "<br><br>";
        if ($response_code != 200) {
            throw new AirshipFailure($response[1], $response_code);
        }
    }
    
    // Broadcast this rich payload to all users.
    public function broadcastRichPush($payload, $exclude_tokens=null) {
        if ($exclude_tokens != null) {
            $payload['exclude_tokens'] = $exclude_tokens;
        }
        $body = json_encode($payload);
        $response = $this->_request(RICH_BROADCAST_URL, 'POST', $body, 'application/json');
        $response_code = $response[0];
        
		echo "Rich Push Broadcast Status Code: " . $response_code . "<br><br>";
        if ($response_code != 200) {
            throw new AirshipFailure($response[1], $response_code);
        }
    }
    ...
    public function echoStatusCode($descriptor="Text", $code=0) {
    	echo $descriptor . " " . $code;
    }
}
